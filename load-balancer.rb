...
class LoadBalancer < Controller
  ...
  def get_server_list(dpid)
    for i in START_SERVER..LAST_SERVER
      target_ip_addr = "192.168.0.#{i}"
      arp_request_message = Pio::Arp::Request.new(:source_mac => '00:00:00:00:00:00',
                                                  :sender_protocol_address => '192.168.0.127',
                                                  :target_protocol_address => target_ip_addr)
      flood(dpid, arp_request_message.to_binary)
  ...
  def handle_ipv4(dpid, message, port)
    ...
    if to_server
      if port
        action = [Trema::SetIpDstAddr.new(@next_server),
      ...
      if port
        action = [Trema::SetIpSrcAddr.new(@original_dst_server),
